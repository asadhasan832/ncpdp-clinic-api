module.exports = {
  ...
  friendlyName: 'Deactivate clinic',
  ...
  fn: async function (inputs) {
    ...
    sails.log('Usage: sails run deactivate-clinic CLINIC_ID');
    if(_.isUndefined(process.argv[4])) {
      throw new Error('Usage: sails run deactivate-clinic CLINIC_ID');
    } else {
      var c = await Clinic.update({id: process.argv[4]}).set({deactivated: true, serverOSUser: require("os").userInfo().username, serverOSUserID: require("os").userInfo().uid}).fetch();
      delete c.apiKey;
      delete c.apiSecret;
      sails.log(c);
    }
